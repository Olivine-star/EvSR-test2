...
class DualBranchSNN(torch.nn.Module):
    def __init__(self, netParams):
        super(DualBranchSNN, self).__init__()
        self.pos_branch = NetworkBasic(netParams)
        self.neg_branch = NetworkBasic(netParams)

    def forward(self, spikeInput):
        # spikeInput: [B, 2, H, W, T] -> split along channel
        ...
        return torch.cat([out_pos, out_neg], dim=1)  # [B, 2, H', W', T]  合并后
