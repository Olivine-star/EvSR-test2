...
def main():
    ...
    for epoch in range(epoch0 + 1, maxEpoch):
        ...
        for i, (eventLr, eventHr) in enumerate(trainLoader, 0):
            ...
            """
            修改"""
            eventLr, eventHr = eventLr.to(device), eventHr.to(device)  # eventLr.shape = [B, 5, 2, H, W, T]

            # 对每个子流做前向推理
            outputs = [m(eventLr[:, i]) for i in range(eventLr.shape[1])]  # List of [B, 2, H, W, T]
            output = torch.stack(outputs, dim=0).mean(dim=0)  # shape: [B, 2, H, W, T]

            # 对 ground truth 也取平均（融合5个子流）
            eventHr = eventHr.mean(dim=1)  # shape: [B, 2, H, W, T]
    ...
    loss_plot_path = os.path.join(savePath, "loss_curve.png")
